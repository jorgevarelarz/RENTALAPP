...
`app.rentalapp.es` must have an A record to `5.250.186.153` before TLS can be issued.
The root `rentalapp.es` A records are provider-managed, and `www`/`api` currently point to Railway.
