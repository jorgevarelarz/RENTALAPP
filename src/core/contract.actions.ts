...
interface CreateContractParams {
  ...
  iban?: string;
  region?: string;
  clauses?: ClauseInput[] | Record<string, any>;
...
export const createContractAction = async (params: CreateContractParams) => {
  ...
  if (Array.isArray(clauses)) {
    ...
    } else if (normalizedRegion !== 'general') {
  ...
  const pdfClauses = Array.isArray(contract.clauses)
    ? contract.clauses.map(c => ({ id: c.id, text: c.text, params: c.params }))
    : contract.clauses;
  ...
  try {
    ...
    logger.error({ err: notifyErr, contractId: contract.id }, 'Error enviando notificaciones de contrato creado');
  }
...
export const signContractAction = async (contractId: string, user: { id: string; role: string }) => {
  ...
  try {
    ...
    logger.error({ err: notifyErr, contractId: contract.id }, 'Error enviando notificaciones de firma');
  }
